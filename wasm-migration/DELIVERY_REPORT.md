> Current application integration: see the root README. The frozen oracle is now `wasm-migration/oracle/sudoku.html`; the single root WASM HTML is the application entry. Earlier delivery-only statements below describe the historical pre-integration package.
