...
```js
import { instantiateCore, loadPosition, loadGivenGrid, findNextStepWasm }
...
入口验收：`npm run test:coarse`；soundness 负例：`npm run test:soundness`。完整 trace：`npm run test:trace`，它按题复用哈希匹配的 PASS，避免重跑已完成题。原有差分脚本保留；详见 BUILD.md。浏览器 Worker 脚本需安装 Playwright 并提供其 index.mjs 的绝对路径，使用本机 Edge：`node tests/browser-worker.mjs <playwright/index.mjs>`。
